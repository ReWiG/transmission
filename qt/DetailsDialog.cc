...
void DetailsDialog::refreshUI()
{
    ...
    if (canEdit() && !torrents.empty())
    {
        ...
        // mySequentialCheck
        uniform = true;
        baseline_flag = baseline.sequentialDownload();
        for (Torrent const* const tor : torrents)
        {
            if (baseline_flag != tor->sequentialDownload())
            {
                uniform = false;
                break;
            }
        ...
        ui.sequentialCheck->setChecked(uniform && baseline_flag);
...
{
    mySession.torrentSet(TR_KEY_sequentialDownload, val);
}
